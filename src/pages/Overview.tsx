  ...
  return <section className="overview-page">
    ...
    {!loading && !error && dashboard && <>
      ...
      <div className="overview-metrics">
        <article><span><CalendarIcon /></span><div><strong>{dashboard.summary.session_count}</strong><small>Detection Sessions วันนี้</small></div></article>
        <article><span><LuClock3 /></span><div><strong>{formatMinutes(dashboard.summary.total_duration_seconds)}</strong><small>เวลาที่ตรวจพบการใช้งาน</small></div></article>
        <article><span><LuCalendarDays /></span><div><strong>{activePlanCount}</strong><small>แผนที่กำลังใช้งาน</small></div></article>
      ...
      <div className="overview-columns">
        <section className="overview-panel active-plan-panel"><div className="overview-panel-heading"><div><h2>แผนที่กำลังใช้งาน</h2><p>แผนดูแลดวงตาปัจจุบันของคุณ</p></div><button onClick={() => onNavigate("Plan")}>ดูแผนทั้งหมด<LuArrowRight /></button></div>{activePlan ? <div className="overview-active-plan"><span className="overview-plan-icon"><LuEye /></span><div className="overview-plan-copy"><span className="overview-status">กำลังดำเนินการ</span><h3>{activePlan.plan_name}</h3><p>{activePlan.goal_name}</p><small><LuCalendarDays /> {formatDate(activePlan.start_date)} - {formatDate(activePlan.end_date)}</small><div className="overview-plan-progress"><div><span style={{ width: `${completion}%` }} /></div><strong>{completion}%</strong></div></div><div className="overview-plan-actions"><button onClick={() => onNavigate("Plan")}>ดูแผน</button><button onClick={() => onNavigate("Plan")}>ติดตามผล</button></div></div> : <div className="overview-empty">ยังไม่มีแผนที่กำลังใช้งาน</div>}</section>

        <section className="overview-panel"><div className="overview-panel-heading"><div><h2>ความเสี่ยงล่าสุด</h2><p>ช่วงที่ควรใส่ใจจากข้อมูลวันนี้</p></div><button onClick={() => onNavigate("Dashboard")}>ดูรายละเอียด<LuArrowRight /></button></div>{dashboard.risk_periods[0] ? <div className="overview-risk"><LuShieldCheck /><div><strong>{health.label}</strong><p>{dashboard.risk_periods[0].app_name} · Blink Rate {dashboard.risk_periods[0].blink_rate.toFixed(1)} ครั้ง/นาที</p><small>{new Date(dashboard.risk_periods[0].started_at).toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" })}</small></div></div> : <div className="overview-empty">ไม่พบช่วงที่ต่ำกว่าเกณฑ์ในวันนี้</div>}</section>

        <section className="overview-panel"><div className="overview-panel-heading"><div><h2>การใช้งานแอปล่าสุด</h2><p>เรียงตามเวลาที่ใช้งานวันนี้</p></div><button onClick={() => onNavigate("Dashboard")}>ดูทั้งหมด<LuArrowRight /></button></div><div className="overview-app-list">{dashboard.app_usage.slice(0, 5).map((app) => <div key={app.app_name}><span>{app.app_name.slice(0, 1).toUpperCase()}</span><strong>{app.app_name}</strong><small>{formatMinutes(app.duration_seconds)}</small><em className={app.blink_rate < dashboard.normal_range.min ? "risk" : "normal"}>{app.blink_rate.toFixed(1)}/นาที</em></div>)}{dashboard.app_usage.length === 0 && <div className="overview-empty">ยังไม่มีข้อมูลการใช้งานแอปวันนี้</div>}</div></section>
      </div>
